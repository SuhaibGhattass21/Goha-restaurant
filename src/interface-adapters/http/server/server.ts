...
import { QueryInterceptor } from "../../../infrastructure/database/interceptors/query.interceptor";
...
export class Server {
  ...
  private initializeLogging(): void {
    ...
    // Initialize query interceptor for database logging
    if (AppDataSource.isInitialized) {
      QueryInterceptor.initialize(AppDataSource);
    }

    // Start system monitoring in production
    if (process.env.NODE_ENV === 'production') {
      this.monitoringService.startMonitoring(60000); // Every minute
    }
  ...
  private setupMiddlewares(): void {
    ...
    this.app.use(loggingMiddleware.logAuthEvent);
    this.app.use(loggingMiddleware.logValidationErrors);
    this.app.use(loggingMiddleware.logSecurityEvents);
  }
